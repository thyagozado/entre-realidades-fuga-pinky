...
interface ReturnToMapButtonProps {
  onClick: () => void;
  className?: string; // Allow axtra classes for positioning if needed
}

const ReturnToMapButton: React.FC<ReturnToMapButtonProps> = ({ onClick, className }) => {
  return (
    <button
      onClick={onClick}
      title="Voltar ao Mapa"
      className={`fixed top-5 left-5 z-50 w-12 h-12 p-2 bg-gray-200 hover:bg-gray-300 text-gray-800 border-2 border-gray-700 rounded-md shadow-md transition-colors duration-150 flex items-center justify-center ${className}`}
    >
      {/* SVG Fornecido com ajustes */}
...
export default ReturnToMapButton; 